		...
		try {
			...
			if (Cpu::shown) {
				...
				if (key == "+" and Config::getI("update_ms") <= 86399900) {
					...
					try {
						// 读取可用的调频策略
						vector<string> available_governors;
						if (std::filesystem::exists("/sys/devices/system/cpu/cpufreq/policy0/scaling_available_governors")) {
							std::ifstream gov_file("/sys/devices/system/cpu/cpufreq/policy0/scaling_available_governors");
							string line;
							if (getline(gov_file, line)) {
								std::istringstream iss(line);
								string governor;
								while (iss >> governor) {
									available_governors.push_back(governor);
								}
							}
						}
						...
						// 切换到下一个策略
						if (not available_governors.empty() and not current_governor.empty()) {
							int current_index = 0;
							for (int i = 0; i < (int)available_governors.size(); ++i) {
								if (available_governors[i] == current_governor) {
									current_index = i;
									break;
								}
							}
							...
							// 切换到下一个策略（循环）
							int next_index = (current_index + 1) % available_governors.size();
							string next_governor = available_governors[next_index];
							
							// 执行切换命令
							string command = "sudo bash -c \"echo " + next_governor + " > /sys/devices/system/cpu/cpufreq/policy0/scaling_governor\"";
							[[maybe_unused]] int result = system(command.c_str());
							
							// 更新显示状态
							Cpu::current_governor = next_governor;
							redraw = true;
						}
