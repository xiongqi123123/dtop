...
//* Functions and variables for reading and writing the dtop config file
namespace Config {
	...
	const vector<array<string, 2>> descriptions = {
		{"color_theme", 		"#* Name of a dtop++/bpytop/bashtop formatted \".theme\" file, \"Default\" and \"TTY\" for builtin themes.\n"
								"#* Themes should be placed in \"../share/dtop/themes\" relative to binary or \"$HOME/.config/dtop/themes\""},
		...
		{"log_level", 			"#* Set loglevel for \"~/.config/dtop/error.log\" levels are: \"ERROR\" \"WARNING\" \"INFO\" \"DEBUG\".\n"
	...
	void write() {
		...
		if (cwrite.good()) {
			cwrite << "#? Config file for dtop v. " << Global::Version;
